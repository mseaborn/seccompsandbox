...
# This is a temporary script to convert sandbox.cc.  It will be
...
def munge_lines(data):
    return (
        "// This has been automatically extracted from sandbox.cc.\n"
        "// It will be fixed up in the next commit.\n\n" +
        "".join(munge(line) + "\n" for line in data.split("\n")))


def main():
    data = read_file("sandbox.cc")
    for arch in ("x86_64", "i386"):
        asm = extract_between(data,
                              "\n//CUT_%s_START\n" % arch,
                              "\n//CUT_%s_END\n" % arch)
        write_file("fault_handler_%s.S" % arch, munge_lines(asm))
